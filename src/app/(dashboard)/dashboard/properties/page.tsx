...
import { listings as items } from "@/constants/data";
...
export const TableItem = ({ listing }: { listing: Record<string, any> }) => {
  ...
  return (
    <TableRow key={listing.name}>
      ...
      <TableCell>{listing.tenant?.name}</TableCell>
      <TableCell>
        <Button className="flex gap-3 items-center w-32" disabled={loading}>
          <span> Send Bill</span>
          ...
          {loading && (
            <div role="status">
              <svg
                aria-hidden="true"
                className="w-4 h-4xw text-gray-200 animate-spin dark:text-gray-600 fill-gray-100"
                viewBox="0 0 100 101"
                fill="none"
